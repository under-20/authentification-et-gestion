...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['form-type'] === 'edit') {
    ...
    if (!empty($_FILES['avatar-upload']['name'])) {
        ...
        if (in_array($fileType, ['jpg', 'jpeg', 'png', 'gif'])) {
            if (move_uploaded_file($_FILES["avatar-upload"]["tmp_name"], $targetFile)) {
                // On sauvegarde seulement le chemin relatif
                $avatar = "assets/uploads/avatars/" . $uniqueName;
            }
        }
    }
    ...
    if (!empty($newPwd) && !empty($currentPwd)) {
        if ($currentPwd !== $user['mdps']) {  // ATTENTION : Comparaison simple (normal pour ton projet actuel)
    ...
    // Récupérer l'ancien rôle sans le changer
    $role = $user['role'];
...
<body>
    <div class="admin-profile-container">
        <h1><i class="fas fa-user-cog"></i> Modifier mon profil</h1>
